...
import { getPersonalSpaceData } from "@/lib/data";
...
export default function PersonalPage() {
  return (
    <>
      <Suspense fallback={<TopBar title="Profil" large unread={0} />}>
        <TopBarWithUnread title="Profil" large />
      </Suspense>

      <Suspense fallback={<PersonalBodySkeleton />}>
        <PersonalBody />
      </Suspense>
    </>
  );
...
async function PersonalBody() {
  const data = await getPersonalSpaceData();
...
function PersonalBodySkeleton() {
  return (
    <div className="px-4 pb-28 space-y-6">
      <div className="flex items-center gap-5">
        <Skeleton className="h-[120px] w-[96px]" />
